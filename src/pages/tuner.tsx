...
export default function Tuner() {
    ...
    return (
        <div id="tuner" className="w-full h-auto 2xl:p-4 2xl:p-12 2xl:px-40 !pb-[0] pt-10">
            <div className="px-[15px] sm:px-15 md:px-40 text-center lg:text-left">
            ...
            <div className="flex flex-col md:flex-row flex-wrap items-start mt-6 md:mt-8 gap-2 mg:gap-4 justify-center">
                ...
                <div className="order-1 lg:order-2 w-auto px-[15px] mt-[24px] w-full md:w-auto">
                    <TuningPresets presets={tuningPresetsData} onPresetSelect={setSelectedPreset} />
                </div>

                {/* Guitar & Strings */}
                <div className="order-2 lg:order-1 md:mt-[24px] tuner-container text-center relative w-full md:w-auto">
                    {tuningPresetsData[selectedPreset].set.map((note, index) => (
                    <TunedString key={index} note={note} position={index} />
                    ))}
                    <img
                        className="inline-block mt-[48px] w-[500px] max-w-none lg:max-w-[100%] lg:w-[530px] xl:w-[550px] ml-[20px] sm:ml-[60px] md:ml-30 lg:ml-0"
